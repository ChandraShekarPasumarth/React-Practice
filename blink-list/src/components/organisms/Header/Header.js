    ...
    <div className={classes.root}>
      ...
      <AppBar position="static" className={classes.root}>
        <Toolbar>
          ...
          {value === 2 && (
            <CustomInput
              placeholder="Search for titles, authors and Categories"
              value={searchValue}
              onChange={(event) => searchNew(event.target.value.trim())}
              view={view}
            />
          )}
        </Toolbar>
      ...
      <Container maxWidth="md">
        <div>
          {(value === 1 || value === 2 || value === 0) &&
            ...
            isAuthenticated && (
              <LibraryTabs
                handleCurrentBooks={handleCurrentBooks}
                searchValue={searchValue}
                exploreStatus={statusValue}
              />
            )}
          {value === 1 && (
            <Form
